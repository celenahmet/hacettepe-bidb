...
import tr.edu.hacettepe.bidb.repo.BelgeRepo;
import tr.edu.hacettepe.bidb.repo.SayfaRepo;
import tr.edu.hacettepe.bidb.repo.SayfaSurumRepo;
...
public class YonetimSayfaController {
    ...
    private final SayfaSurumRepo surumler;
    private final BelgeRepo belgeler;
    private final YonlendirmeRepo yonlendirmeler;

    public YonetimSayfaController(SayfaRepo sayfalar, SayfaSurumRepo surumler,
                                  BelgeRepo belgeler, YonlendirmeRepo yonlendirmeler) {
    ...
    private void surumAl(Sayfa s, String aciklama, Authentication kimlik) {
        SayfaSurum sur = new SayfaSurum();
    ...
    public ResponseEntity<SayfaSurum> surum(@PathVariable Long surumId) {
    ...
    public ResponseEntity<?> sayfaEkle(@RequestBody YeniSayfaIstek istek) {
        ...
        if (sayfalar.bul(slug, dil).isPresent()) {
    ...
    public ResponseEntity<?> adresDegistir(@PathVariable Long id, @RequestBody AdresIstek istek) {
        return sayfalar.findById(id).map(s -> {
            ...
            if (!yeni.equals(s.getSlug())) {
                if (sayfalar.bul(yeni, s.getDil()).isPresent()) {
                ...
                Yonlendirme y = yonlendirmeler.findByEskiYol(eskiYol).orElseGet(Yonlendirme::new);
    ...
    public List<Belge> belgeListesi(@PathVariable Long id) {
    ...
    private void aktar(BelgeIstek istek, Belge b) {
        b.setAd(istek.ad());
